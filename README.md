...
* 이 실험의 목적은 OpenVLA가 robot action을 출력하는 inference 과정에서 GPU memory와 throughput을 줄일 수 있는지 확인하는 것이다.
...
- 2. 로봇 관련 dataset이 vision(image)과 같은 다른 dataset에 비해 얻기가 어렵고 적용하기도 어려운 것으로 알고 있다. 저자의 이러한 방법은 vision과 Language 데이터셋만으로 기존의 control policy을 개선할 수 있는 방법이 있지 않을까? 
