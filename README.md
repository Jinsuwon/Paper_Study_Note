...
* CoT-VLA는 visual reasoning step을 명시적으로 추가한다.

* 먼저 현재 observation (s_t)와 language instruction (l)을 보고 (n) frame 뒤의 future image를 예측한다.

```text
\hat{s}_{t+n} \sim P_\theta(s_{t+n} | s_t, l)
```

* 여기서 (\hat{s}_{t+n})은 모델이 생성한 subgoal image이다.

* 이후 모델은 현재 observation, language instruction, subgoal image를 조건으로 action sequence를 생성한다.
...
* 일반적인 VLM이 image understanding에 초점을 둔다면, VILA-U는 image/video generation까지 수행할 수 있다.
...
```text
1. VLA
   = 같은 VILA-U backbone을 사용하지만 CoT reasoning과 action chunking 없음

2. + action chunking
   = single action이 아니라 action sequence를 예측

3. + hybrid attention
   = action sequence prediction에 full attention 추가
